  ...
  const polygons = tileData.reduce((memo, row, rowIndex) => {
    row.forEach((tileId, colIndex) => {
      ...
      memo[tileId].transforms.push({
        translate: [colIndex * tileWidth, rowIndex * tileHeight],
      });
    });
    return memo;
  }, {});
...
export const genInfinityCardData = (tileWidth) => {
  return {
    canvas: {
      ...
      height: tileWidth * infinityCardAscii.trim().split(`\n`).length,
    },
    polygons: polygonsFromAsciiTiles(tileWidth, tileWidth, infinityCardAscii),
  };
};
